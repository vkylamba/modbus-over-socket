...
import utils.fake_serial as serial
...
class Instrument:
    ...
    def _perform_command(self, functioncode, payload_to_slave):
        ...
        # Communicate
        return self.parse_serial_command(request, number_of_bytes_to_read), functioncode

    def get_command_response(self, data, functioncode):
        response = self.parse_serial_response(data)
        # Extract payload
    ...
    def _communicate(self, request, number_of_bytes_to_read):
        ...
        """
        request = self.parse_serial_command(request, number_of_bytes_to_read)
        # Write request
        ...
        return self.parse_serial_response(answer)

    def parse_serial_command(self, request, number_of_bytes_to_read):
        _check_string(request, minlength=1, description="request")
        _check_int(number_of_bytes_to_read)

        self._print_debug(
            """
                Will write to instrument (expecting {} bytes back): {!r} ({})
            """.format(
    ...
    def parse_serial_response(self, answer):
        ...
        if sys.version_info[0] > 2 and not isinstance(answer, str):
