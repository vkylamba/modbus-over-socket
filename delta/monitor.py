...
def send(conn, req, cmd, subcmd, data=b'', addr=1):
    ...
    if DEBUG:
        print(">>> SEND:", binascii.hexlify(msg), "=>", binascii.hexlify(data))
        print(">>>RAW>>>", data)
    # conn.write(data)
    # conn.flush()
...
def main():
    ...
    parser = ArgumentParser(
        description='Delta inverter simulator (slave mode) or datalogger (master mode) for RPI M8A')
        ...
        for data in receive(conn):
            if MODE == 'master' and data['addr'] == args.a and data['req'] in (ACK, NAK,):
                ...
                if d['req'] == ACK:
                    if not (d['cmd'] == 0x60 and d['cmdsub'] == 0x01):
                        print("Can't decode request cmd=0x%02X, cmdsub=0x%02X" % (
                            d['cmd'], d['cmdsub']))
            ...
            if MODE == 'slave' and data['addr'] == args.a and data['req'] in (ENQ,):
                ...
                if d['cmd'] == 0x60 and d['cmdsub'] == 0x01:
                    ...
                    print(
                        "This simulator only replies to cmd=0x60 cmdsub=0x01 requests...")
